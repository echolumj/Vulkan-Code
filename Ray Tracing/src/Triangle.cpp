...
void Triangle::commandPool_create(void)
{
	...
	poolCreateInfo.queueFamilyIndex = queueFamilyIndices.graphicAndComputeFamily.value();
...
void Triangle::recordComputeCommandBuffer(VkCommandBuffer commandBuffer, un)
{
	VkCommandBufferBeginInfo beginInfo{};
	beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
	...
	vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compPipelineLayout, 0, 1, &computeDescriptorSets[i], 0, 0);
...
void Triangle::computeCommandBuffers_create(void)
{
	//command buffer的数量和swapChainFramebuffersframe buffer的数量一致
